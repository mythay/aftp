...
For example:
	aftp put 192.168.1.1 192.168.1.2`,
...
func init() {
	...
	putCmd.Flags().StringVarP(&fUser, "username", "u", "pcfactory", "A valid ftp user name")
	putCmd.Flags().StringVarP(&fPasswd, "password", "p", "pcfactory", "Correspoding ftp password")
	putCmd.Flags().StringVarP(&fDestdir, "directory", "d", "/fw", "Ftp server directory to store the file")
	putCmd.Flags().StringVarP(&fLocalfile, "localfile", "l", "App2.out", "local file path to be uploaded")
	progress.Width = 40
